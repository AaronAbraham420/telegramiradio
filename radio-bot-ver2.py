...
Streams via: Liquidsoap (Podman) → Icecast2
"""
...
def require_env(key: str) -> str:
    ...
    if not val:
        print(f"❌ Missing required env var: {key} — check your .env file")
        sys.exit(1)
    return val
...
# Liquidsoap / Icecast
LIQUIDSOAP_HOST   = os.environ.get("LIQUIDSOAP_HOST", "127.0.0.1")
...
PUBLIC_STREAM_URL = os.environ.get("PUBLIC_STREAM_URL", f"http://{ICECAST_HOST}:{ICECAST_PORT}{ICECAST_MOUNT}")
# ─────────────────────────────────────────
...
async def liquidsoap_command(command: str) -> str | None:
    """
    Send a command to Liquidsoap via async telnet.
    Returns the response string or None on failure.
    Works with Liquidsoap running in Podman via --network host.
    """
    try:
        ...
        log.error(f"Liquidsoap telnet timeout — is the container running? (podman ps)")
        return None
    except ConnectionRefusedError:
        log.error(f"Liquidsoap telnet refused on {LIQUIDSOAP_HOST}:{LIQUIDSOAP_PORT} — check podman-compose up")
        return None
    except Exception as e:
        log.error(f"Liquidsoap command failed: {e}")
        return None

async def push_to_liquidsoap(audio_url: str) -> bool:
    """Push an audio URL into the Liquidsoap request queue."""
    response = await liquidsoap_command(f"queue.push {audio_url}")
    if response is not None:
        log.info(f"Liquidsoap queued: {audio_url} → {response}")
        return True
    return False

async def skip_liquidsoap() -> bool:
    """Skip the current track in Liquidsoap."""
    response = await liquidsoap_command("queue.skip")
    return response is not None

async def check_liquidsoap_alive() -> bool:
    """Returns True if Liquidsoap telnet is reachable."""
    response = await liquidsoap_command("version")
    return response is not None
...
async def cmd_play(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    liquidsoap_status = ""
    if url:
        pushed = await push_to_liquidsoap(url)
        if pushed:
            liquidsoap_status = "\n📻 Sent to stream!"
        else:
            liquidsoap_status = "\n⚠️ Queued locally but Liquidsoap unreachable — run `podman-compose up -d`"
    ...
    if url:
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("▶ Play Now", callback_data=f"playnow:{url}"),
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("lyrics:"):
        ...
        audio_url = data[len("playnow:"):]
        if audio_url:
            pushed = await push_to_liquidsoap(audio_url)
            if pushed:
                await query.message.reply_text("▶ Pushed to stream now!")
            else:
                await query.message.reply_text("⚠️ Could not reach Liquidsoap — is `podman-compose up -d` running?")
        else:
            await query.message.reply_text("⚠️ No audio URL available for this track.")


async def cmd_stream(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Send the live stream URL."""
    await update.message.reply_text(
        f"📻 *Live Stream*\n`{PUBLIC_STREAM_URL}`\n\n"
        "Open in VLC, browser, or any media player.\n"
        "Mobile: paste into VLC → Network Stream",
        parse_mode="Markdown"
    )


async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Check if Liquidsoap and Icecast are reachable."""
    ls_alive = await check_liquidsoap_alive()
    ls_icon  = "✅" if ls_alive else "❌"

    # Check Icecast via HTTP
    icecast_alive = False
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                icecast_alive = r.status == 200
    except Exception:
        pass
    ic_icon = "✅" if icecast_alive else "❌"

    await update.message.reply_text(
        f"🔧 *System Status*\n\n"
        f"{ls_icon} Liquidsoap (`{LIQUIDSOAP_HOST}:{LIQUIDSOAP_PORT}`)\n"
        f"{ic_icon} Icecast (`{ICECAST_HOST}:{ICECAST_PORT}`)\n\n"
        f"📻 Stream: `{PUBLIC_STREAM_URL}`\n"
        f"🎵 Queue: {len(queue.items)} song(s) waiting\n"
        f"▶ Now playing: {queue.current.get('title','—') if queue.current else '—'}",
        parse_mode="Markdown"
    )
...
async def main():
    ...
    log.info(f"Stream URL: {PUBLIC_STREAM_URL}")
    log.info(f"Liquidsoap telnet: {LIQUIDSOAP_HOST}:{LIQUIDSOAP_PORT}")

    # Check Liquidsoap on startup
    alive = await check_liquidsoap_alive()
    if alive:
        log.info("✅ Liquidsoap is reachable")
    else:
        log.warning("⚠️  Liquidsoap not reachable — run: podman-compose up -d")
